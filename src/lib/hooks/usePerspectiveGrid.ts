...
export function usePerspectiveGrid(
        ...
        setState((prev) => {
            ...
            if (updates.type === 1) {
                newVPs = newVPs.map(vp => {
                    if (vp.id === "vp1" && Math.abs(vp.distanceFromCenter) > prev.canvasWidth * 0.5) {
     ...
     */
    const setReferenceImage = useCallback((file: File) => {
    ...
    const handleMouseDown = useCallback(
        (e: React.MouseEvent<HTMLCanvasElement>) => {
            ...
            } else if (state.referenceImage?.isInteractive && state.referenceImage?.isVisible) {
    ...
    const handleMouseMove = useCallback(
        (e: React.MouseEvent<HTMLCanvasElement>) => {
            ...
            if (isDragging && initialDragState.current) {
                if (isDragging === "ref_handle") {
                    ...
                    const gridDeltaX = (screenDeltaX * Math.cos(angleRad) - screenDeltaY * Math.sin(angleRad)) / zoom;
                    const gridDeltaY = (screenDeltaX * Math.sin(angleRad) + screenDeltaY * Math.cos(angleRad)) / zoom;
        ...
        [state, isDragging, isPanning, updatePan, canvasRef, updateReferenceImage, setState]
    );
                ...
                if (prevDragging === "ref_handle") {
                    ...
                    if (Math.abs(initialX) < 0.1 && Math.abs(initialY) < 0.1) {
                        ...
                        const animateRefHandle = (currentTime: number) => {
                            ...
                            if (progress < 1) {
                                refHandleAnimFrameRef.current = requestAnimationFrame(animateRefHandle);
                            } else {
